...
| 구분      | 공통점                       | 차이점                                         |
| ------- | ------------------------- | ------------------------------------------- |
| 타입      | 둘 다 사용자 정의 데이터 타입         | 구조체는 값 타입, 클래스는 참조 타입                       |
| 프로퍼티    | 변수와 상수를 가질 수 있음           | 구조체는 값 복사, 클래스는 참조 복사                       |
| 메서드     | 함수(메서드)를 가질 수 있음          | 구조체는 메서드 내에서 `mutating` 필요, 클래스는 불필요        |
| 초기화     | 기본 생성자 자동 제공              | 구조체는 기본 생성자 자동 제공, 클래스는 상속 시 생성자 관리가 더 복잡   |
| 프로토콜 준수 | 둘 다 프로토콜 채택 가능            | 없음                                          |
| 상속      | 둘 다 상속 불가능 (X) 아니면 가능 (O) | 구조체는 상속 불가능, 클래스는 상속 가능                     |
| 메모리 관리  | 모두 메모리 할당 필요              | 구조체는 스택 또는 최적화된 위치에 할당, 클래스는 힙에 할당 및 ARC 관리 |
| 참조 비교   | 직접 비교 연산자 구현 가능           | 클래스는 `===`로 참조 비교 가능, 구조체는 값 비교만 가능         |
